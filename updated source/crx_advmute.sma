...
public _crx_is_user_muted(iPlugin, iParams)
{
	enum
	{
		arg_index_receiver = 1,
		arg_index_sender = 2
	}
	new iReceiver = get_param(arg_index_receiver)
	new iSender = get_param(arg_index_sender)

	return get_mute(iReceiver, iSender)
}
